// OTEL bootstrap (SFU-12) — copied from sync.sideby.me/src/server/telemetry/bootstrap.ts,
// with the service.name default set to sfu.sideby.me and the OTLP endpoint pointed
// cross-host (D-13). Keeps the graceful-skip-when-endpoint-unset behavior and the
// parseOtelHeaders support so the cross-host endpoint can carry a bearer/auth header.
...
import { disableSfuTelemetryLogs, enableSfuTelemetryLogs } from './logs.js';
...
export function resolveTelemetryResourceAttributes(env: TelemetryEnv = process.env): Record<string, string> {
  ...
  return {
    'service.name': env.OTEL_SERVICE_NAME?.trim() || 'sfu.sideby.me',
...
export async function initializeTelemetry(options: InitializeTelemetryOptions = {}): Promise<void> {
  ...
  if (!endpoint) {
    disableSfuTelemetryLogs();
    logger.info?.('sfu telemetry bootstrap skipped: OTEL_EXPORTER_OTLP_ENDPOINT is not configured');
    return;
  }
  ...
  try {
    ...
    await starter(sdk);
    enableSfuTelemetryLogs(resourceAttributes['service.version']);

    logger.info?.('sfu telemetry bootstrap initialized', {
      traceUrl,
      metricUrl,
      logUrl,
    });
    ...
    disableSfuTelemetryLogs();
    logger.warn('sfu telemetry bootstrap failed; continuing without exporter', {
